...
def _env_files() -> tuple[Path, ...]:
    ...
    files: list[Path] = [
        Path.home() / ".config" / "hyper-claude-code" / ".env",
        Path(".env"),
    ]
...
class Settings(BaseSettings):
    ...
    cache_max_entries: int = Field(
        default=1000, validation_alias="CACHE_MAX_ENTRIES"
    )
    enable_cost_tracking: bool = Field(
        default=False, validation_alias="ENABLE_COST_TRACKING"
    )
    max_session_cost_usd: float | None = Field(
        default=None, validation_alias="MAX_SESSION_COST_USD"
    )
    storage_db_path: str = Field(
        default="~/.fcc/hcc.db", validation_alias="STORAGE_DB_PATH"
    )

    # ==================== Provider Failover ====================
    enable_provider_failover: bool = Field(
        default=False, validation_alias="ENABLE_PROVIDER_FAILOVER"
    )
    failover_error_threshold: float = Field(
        default=0.5, validation_alias="FAILOVER_ERROR_THRESHOLD"
    )
    failover_window_size: int = Field(
        default=10, validation_alias="FAILOVER_WINDOW_SIZE"
    )
    failover_cooldown_seconds: float = Field(
        default=30.0, validation_alias="FAILOVER_COOLDOWN_SECONDS"
    )
    failover_chains: dict[str, list[str]] = Field(
        default_factory=dict, validation_alias="FAILOVER_CHAINS"
    )
    ...
    max_concurrent_sessions: int = Field(
        default=10, validation_alias="MAX_CONCURRENT_SESSIONS"
    )
