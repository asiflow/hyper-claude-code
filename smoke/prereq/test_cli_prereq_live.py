...
from smoke.lib.child_process import cmd_fcc_init, cmd_hyper_claude_code_serve
...
def test_fcc_init_scaffolds_user_config(
    ...
    assert (tmp_path / ".config" / "hyper-claude-code" / ".env").is_file()


def test_hyper_claude_code_entrypoint_starts_server(smoke_config: SmokeConfig) -> None:
    with start_server(
        smoke_config,
        command=cmd_hyper_claude_code_serve(),
