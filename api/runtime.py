...
from middleware import Pipeline
...
def warn_if_process_auth_token(settings: Settings) -> None:
    ...
    if not settings.anthropic_auth_token and settings.host in ("0.0.0.0", "::"):
        logger.warning(
            "ANTHROPIC_AUTH_TOKEN is empty and host is {}. The proxy is "
            "accessible on all interfaces with NO authentication. Set "
            "ANTHROPIC_AUTH_TOKEN to require an API key.",
            settings.host,
        )
...
class AppRuntime:
    ...
    _storage: Storage | None = field(default=None, init=False)
    ...
    async def startup(self) -> None:
        ...
        if self.settings.enable_middleware_pipeline:
            self._storage = Storage(self.settings.storage_db_path)
            await self._storage.init()
            self._pipeline = Pipeline()
            self._pipeline.add(
                FailoverMiddleware(self.settings, self._storage)
            )
            self._pipeline.add(
                ResponseCacheMiddleware(self.settings, self._storage)
            )
            self._pipeline.add(
                RequestLoggerMiddleware(self.settings, self._storage)
            )
            self._pipeline.add(
                CostTrackerMiddleware(self.settings, self._storage)
            )
            self.app.state.storage = self._storage
            self.app.state.pipeline = self._pipeline
        else:
            self.app.state.storage = None
    ...
    async def shutdown(self) -> None:
        ...
        if self.message_handler is not None:
            try:
                await self.message_handler.session_store.flush_pending_save()
            except Exception as e:
                if verbose:
        ...
        await self._shutdown_search_client()
    ...
    async def _shutdown_search_client(self) -> None:
        verbose = self.settings.log_api_error_tracebacks
        try:
            from api.web_tools.outbound import close_search_client
        except Exception:
            return
        await best_effort(
            "close_search_client",
