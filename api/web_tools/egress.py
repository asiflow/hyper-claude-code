...
_CLOUD_METADATA_ADDRS = frozenset(
    {
...
def get_validated_stream_addrinfos_for_egress(
    ...
    try:
        if ipaddress.ip_address(host) in _CLOUD_METADATA_ADDRS:
            raise WebFetchEgressViolation(
                f"Cloud metadata IP {host!r} is always blocked for web_fetch"
            )
    except ValueError:
        pass
    ...
    if policy.allow_private_network_targets:
        ...
        for *_, sockaddr in infos:
            ...
            try:
                ...
                continue
            if resolved in _CLOUD_METADATA_ADDRS:
                raise WebFetchEgressViolation(
                    f"Cloud metadata IP {addr!r} is always blocked for web_fetch"
                )
        return infos
