...
async def _get_search_client() -> httpx.AsyncClient:
    """Return a reusable httpx client for web search requests."""
...
async def close_search_client() -> None:
    """Close the shared search client. Call during shutdown."""
    global _search_client
...
) -> tuple[ClientSession, TCPConnector]:
    """Execute one DNS-pinned fetch hop. Caller must close connector/session."""
    addr_infos = await asyncio.to_thread(
        get_validated_stream_addrinfos_for_egress, url, egress
    )
    host = urlparse(url).hostname or ""
    results = getaddrinfo_rows_to_resolve_results(host, addr_infos)
    resolver = _PinnedEgressStaticResolver(results)
    connector = TCPConnector(resolver=resolver, force_close=True)
...
async def _run_web_fetch(url: str, egress: WebFetchEgressPolicy) -> dict[str, str]:
    ...
    while True:
        session, connector = await _fetch_single_hop(current_url, egress, timeout)
        try:
            async with (
                session,
