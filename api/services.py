...
# Derived from the provider catalog's transport_type field.
def _build_openai_chat_ids() -> frozenset[str]:
...
_OPENAI_CHAT_UPSTREAM_IDS = _build_openai_chat_ids()
...
class ClaudeProxyService:
    ...
    def create_message(self, request_data: MessagesRequest) -> object:
        ...
        return self._create_message_inner(request_data)

    async def create_message_with_pipeline(
        self, request_data: MessagesRequest
    ) -> object:
        """Route through the middleware pipeline, then call create_message."""
        if self._pipeline is not None:
            return await self._pipeline.process(
                request_data, self._create_message_inner
            )
        return self._create_message_inner(request_data)

    def _create_message_inner(self, request_data: MessagesRequest) -> object:
        """Core message handling (called directly or via pipeline)."""
        try:
            ...
            effective_provider_id = getattr(
                request_data, "_failover_provider_id", None,
            ) or routed.resolved.provider_id
            provider = self._provider_getter(effective_provider_id)
