...
class TelegramPlatform(MessagingPlatform):
    ...
    def __init__(
        ...
        *,
        ...
        if not self.allowed_user_id:
            logger.warning(
                "ALLOWED_TELEGRAM_USER_ID is not set — the Telegram bot will "
                "accept messages from ANY user. Set this to restrict access."
            )
    ...
    async def start(self) -> None:
        ...
        if not self.allowed_user_id:
            raise ValueError(
                "ALLOWED_TELEGRAM_USER_ID must be set when using Telegram platform"
            )
