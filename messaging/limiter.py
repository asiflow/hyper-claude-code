...
class MessagingRateLimiter:
        ...
        while not self._shutdown.is_set():
            try:
                ...
                if self._paused_until > now:
                    ...
                    logger.warning(
                        "Limiter worker paused, waiting {:.1f}s more...",
                        wait_time,
                    )
                    ...
                    try:
                        ...
                        if "flood" in error_msg or "wait" in error_msg:
                            ...
                            try:
                                ...
                                logger.debug("Failed to parse retry-after: {}", exc)
                            ...
                            self._paused_until = (
                                asyncio.get_running_loop().time() + wait_secs
                            )
                        else:
    ...
    async def shutdown(self, timeout: float = 2.0) -> None:
        ...
        try:
            ...
            logger.debug(
                "Limiter condition notify during shutdown: {}", type(exc).__name__
            )
