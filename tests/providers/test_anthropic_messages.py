...
def test_default_request_body_strips_internal_fields(provider_config):
    ...
    body = provider._build_request_body(MockRequest())  # ty:ignore[invalid-argument-type]
...
def test_default_request_body_preserves_thinking_budget(provider_config):
    ...
    body = provider._build_request_body(req)  # ty:ignore[invalid-argument-type]
...
async def test_stream_uses_retry_builds_request_and_closes_response(
    ...
    with (
        ...
        events = [event async for event in provider.stream_response(req)]  # ty:ignore[invalid-argument-type]
...
async def test_stream_maps_non_200_to_error_event_and_closes_response(
    ...
    with (
        ...
        events = [
            event
            async for event in provider.stream_response(req, request_id="REQ_123")  # ty:ignore[invalid-argument-type]
        ]
...
async def test_midstream_error_closes_open_block_and_uses_fresh_content_index(
    ...
    with (
        ...
        events = [e async for e in provider.stream_response(req)]  # ty:ignore[invalid-argument-type]
