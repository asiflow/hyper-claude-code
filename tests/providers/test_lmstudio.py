...
async def test_stream_response_omits_thinking_when_globally_disabled(lmstudio_config):
    ...
    with (
        ...
        [e async for e in provider.stream_response(req)]  # ty:ignore[invalid-argument-type]
...
def test_build_request_body_disabled_thinking_strips_native_thinking_history(
    ...
    body = provider._build_request_body(req, thinking_enabled=False)  # ty:ignore[invalid-argument-type]
...
def test_build_request_body_preserves_signed_thinking_native_history(lmstudio_config):
    ...
    body = provider._build_request_body(req, thinking_enabled=True)  # ty:ignore[invalid-argument-type]
