...
def _aiohttp_response(
    ...
    *,
    ...
    r.content = MagicMock()
    r.content.iter_chunked = iter_chunked
    return r


def _aiohttp_session_with_responses(
    *responses: MagicMock,
) -> tuple[MagicMock, MagicMock]:
    """Build a mock session/connector pair for patching ``_fetch_single_hop``."""
...
async def test_run_web_fetch_follows_redirect_when_each_hop_is_allowed():
    ...
    session, connector = _aiohttp_session_with_responses(res_redirect, res_ok)

    async def fake_hop(url, egress, timeout):
        return session, connector

    with patch("api.web_tools.outbound._fetch_single_hop", side_effect=fake_hop):
...
async def test_run_web_fetch_truncates_large_body_to_byte_cap(monkeypatch):
    ...
    session, connector = _aiohttp_session_with_responses(res_ok)
...
async def test_run_web_fetch_redirect_to_blocked_host_raises():
    ...
    session, connector = _aiohttp_session_with_responses(res_redirect)
    call_count = 0
    ...
    async def fake_hop(url, egress, timeout):
        ...
        raise WebFetchEgressViolation(
            "Non-public IP host '127.0.0.1' is not allowed for web_fetch"
        )

    with (
        patch("api.web_tools.outbound._fetch_single_hop", side_effect=fake_hop),
...
async def test_run_web_fetch_redirect_without_location_raises():
    ...
    session, connector = _aiohttp_session_with_responses(res_bad)
...
async def test_run_web_fetch_excess_redirects_raises():
    ...
    session, connector = _aiohttp_session_with_responses(res1, res2)
