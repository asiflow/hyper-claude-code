...
class GlobalRateLimiter:
    ...
    def get_instance(
        ...
        """Get or create the singleton instance, rebuilding on config change."""
        desired_rate_limit = rate_limit or 40
        desired_rate_window = float(rate_window or 60.0)
        if cls._instance is not None and cls._instance.matches_config(
            desired_rate_limit, desired_rate_window, max_concurrency
        ):
            return cls._instance
        if cls._instance is not None:
            logger.info("Rebuilding global rate limiter for updated config")
        cls._instance = cls(
