...
def serve() -> None:
    """Start the FastAPI server (registered as `hyper-claude-code` script)."""
    import uvicorn
...
def init() -> None:
    """Scaffold config at ~/.config/hyper-claude-code/.env (registered as `fcc-init`)."""
    config_dir = Path.home() / ".config" / "hyper-claude-code"
    ...
    print(
        "Edit it to set your API keys and model preferences, then run: hyper-claude-code"
    )
